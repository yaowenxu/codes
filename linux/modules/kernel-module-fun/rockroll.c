...
MODULE_DESCRIPTION("rockroll module");

static char *rockroll_filename = "/home/bork/media/music/rock Astley - Never Gonna Give You Up.mp3";
 ...
 * the module.
 */
module_param(rockroll_filename, charp, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(rockroll_filename, "The location of the rock roll file");
...
asmlinkage long rockroll_open(const char __user *filename, int flags, umode_t mode);

asmlinkage long (*original_sys_open)(const char __user *, int, umode_t);
asmlinkage unsigned long **sys_call_table;


static int __init rockroll_init(void)
{
    if(!rockroll_filename) {
	printk(KERN_ERR "No rock roll filename given.");
    ...
    sys_call_table[__NR_open] = (unsigned long *) rockroll_open;
 ...
 * file name ends with .mp3, in which case it opens the rock roll file instead.
 */
asmlinkage long rockroll_open(const char __user *filename, int flags, umode_t mode)
{
	 ...
	 * and place the rockroll filename at the location the user code passed
	 * in, saving and restoring the memory we overwrite.
	 */
	old_fs = get_fs();
	set_fs(KERNEL_DS);

	/* Open the rockroll file instead */
	fd = (*original_sys_open)(rockroll_filename, flags, mode);
...
static void __exit rockroll_cleanup(void)
{
...
module_init(rockroll_init);
module_exit(rockroll_cleanup);
